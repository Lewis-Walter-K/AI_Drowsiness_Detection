...
MODEL_PATH = './Outputs/drowsiness_model_best.h5'
...
try:
    ...
    print("Loaded Successfully, Turning camera on...")
except Exception as e:
    print(f"ERROR, files are not found, recheck the path!\n{e}")
    exit()

# Label
CLASSES = ['Awake', 'Drowsy', 'Yawn', 'Passed_out']
COLORS = [(0, 255, 0), (0, 165, 255), (0, 255, 255), (0, 0, 255)] # Green, Orange, Yellow, Red

# Tên cột chuẩn xác như trong Scaler của bạn (Quan trọng để xóa Warning)
FEATURE_NAMES = ['EAR', 'MAR', 'PITCH', 'YAW', 'ROLL', 'D_SLUMP', 'R_TILT']
...
def run_demo(camera_index=0):
    ...
    with mp_face_mesh.FaceMesh(refine_landmarks=True, max_num_faces=1) as face_mesh, \
        ...
        while True:
            ...
            features = np.array([[ear, mar, pitch, yaw, roll, d_slump, r_tilt]])
            ...
            try:
                ...
                # Trường hợp không detect được mặt hoặc lỗi tính toán
                label = "No Face"
                confidence = 0
                color = (100, 100, 100)
            ...
            lines = [
                ...
                f"Roll: {roll:.1f}" if roll != DUMMY_VALUE else "Roll: N/A",
                f"Slump: {d_slump:.2f}" if d_slump != DUMMY_VALUE else "Slump: N/A",
                f"R_Tilt: {r_tilt:.1f}" if r_tilt != DUMMY_VALUE else "R_Tilt: N/A",
            ]
            
            # Vẽ nền đen mờ cho text dễ đọc
            cv2.rectangle(overlay, (5, 5), (250, 30 + len(lines)*25), (0, 0, 0), -1)
            
            for i, line in enumerate(lines):
                cv2.putText(overlay, line, (10, 30 + i*25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)

            # 2. HÒA TRỘN (Blending) Overlay vào Frame
            # alpha=0.6 nghĩa là giữ 60% ảnh gốc, 40% ảnh vẽ thêm (giúp lưới mặt trông mờ ảo, công nghệ cao hơn)
            output_frame = cv2.addWeighted(overlay, 0.4, frame, 0.6, 0)

            # 3. Vẽ Kết quả dự đoán (To và Rõ) lên ảnh cuối cùng
            cv2.putText(output_frame, f"STATUS: {label}", (20, h - 50),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.2, color, 3)
            cv2.putText(output_frame, f"Conf: {confidence*100:.1f}%", (20, h - 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)

            # 4. Show
            cv2.imshow('Drowsiness Detection AI (Advanced)', output_frame)
